...
No build step. No dependencies. Open the repo and start editing.

```bash
git clone https://github.com/projekta2/pr-review-canvas.git
cd pr-review-canvas
# Open checklists/interactive-checklist.html in your browser — that's it.
```

Most contributions here are **Markdown files and JSON** — no JavaScript required unless you're touching the interactive checklist.

---

## Start here: tasks you can finish today

If you've never contributed to an open-source project before, or you just want something concrete to pick up without having to read the whole codebase, these are real gaps that need filling:

| Task | File to edit | Time | What to do |
|---|---|---|---|
| Add VS Code snippets | `resources/vscode-snippets.json` | 30–60 min | Create the JSON file following the format in [issue #10](../../issues/10) |
| Add a checklist item you've actually used | `checklists/pr-review-checklist.md` | 20 min | Add the item under the right category + one sentence explaining the failure mode it prevents |
| Fix a phrase that feels vague or preachy | `guides/how-to-give-feedback.md` | 20 min | Replace it with a concrete example from a real review (anonymized) |
| Translate a file to your language | Any file under `checklists/`, `guides/`, `templates/`, or `resources/` | 30–45 min | Mirror the file under `i18n/<lang-code>/` keeping the filename identical |
| Add a team workflow template | `resources/` | 30–45 min | Adapt the checklist format to Linear, Jira, GitHub Projects, or similar tools |
| Report a broken link or typo | Anywhere | 5 min | Open an issue — no PR needed |
